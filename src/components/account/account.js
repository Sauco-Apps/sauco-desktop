...
const Account = ({
    ...
    return ( <
        ...
        /span> <
        p className = "inner primary peer-network" > { peers.data.options.name } <
        /p> <
        p className = "inner secondary peer" > { peers.data.currentPeer } <
        ...
        div className = { `${grid['col-sm-12']} ${grid['col-xs-8']}` } >
        <
        ClickToSend rawAmount = { Math.max(0, account.balance - toRawLsk(0.1)) } >
        <
        div className = { styles['value-wrapper'] } >
        <
